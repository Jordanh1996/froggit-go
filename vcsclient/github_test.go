...
func TestGitHubClient_UploadScanningAnalysis(t *testing.T) {
	...
	sarifID, err := client.UploadCodeScanning(ctx, owner, repo1, "master", testSarifScanContent)
	assert.NoError(t, err)
	assert.Equal(t, "", sarifID)

	_, err = createBadGitHubClient(t).UploadCodeScanning(ctx, owner, repo1, "master", testSarifScanContent)
	assert.Error(t, err)
}

func TestGitHubClient_UploadScanningWithRef(t *testing.T) {
	ctx := context.Background()
	client, cleanUp := createServerAndClient(t, vcsutils.GitHub, false, nil,
		fmt.Sprintf("/repos/%s/%s/code-scanning/sarifs", owner, repo1), createGitHubSarifUploadWithRefHandler)
	defer cleanUp()

	sarifID, err := client.UploadCodeScanningWithRef(ctx, owner, repo1, "refs/pull/123/head", "66d9a06b02a9f3f5fb47bb026a6fa5577647d96e", testSarifScanContent)
	assert.NoError(t, err)
	assert.Equal(t, "", sarifID)

	_, err = createBadGitHubClient(t).UploadCodeScanningWithRef(ctx, owner, repo1, "refs/pull/123/head", "66d9a06b02a9f3f5fb47bb026a6fa5577647d96e", testSarifScanContent)
	assert.Error(t, err)
}
...
func handleSarifUploadRequest(t *testing.T, w http.ResponseWriter, r *http.Request, expectedCommitSHA string, expectedRef string) {
	body, err := io.ReadAll(r.Body)
	assert.NoError(t, err)
	bodyAsString := string(body)
	if !strings.Contains(bodyAsString, expectedCommitSHA) {
		assert.Fail(t, "Unexpected Commit SHA")
	}
	if expectedRef != "" && !strings.Contains(bodyAsString, expectedRef) {
		assert.Fail(t, "Unexpected Ref")
	}
...
func createGitHubSarifUploadHandler(t *testing.T, _ string, _ []byte, _ int) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		case "/repos/jfrog/repo-1/code-scanning/sarifs":
			handleSarifUploadRequest(t, w, r, resultSHA, "")
		default:
...
func createGitHubSarifUploadWithRefHandler(t *testing.T, _ string, _ []byte, _ int) http.HandlerFunc {
	resultSHA := "66d9a06b02a9f3f5fb47bb026a6fa5577647d96e"
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		switch r.RequestURI {
		case "/repos/jfrog/repo-1/code-scanning/sarifs":
			handleSarifUploadRequest(t, w, r, resultSHA, expectedRef)
		default:
			assert.Fail(t, "Unexpected Request URI", r.RequestURI)
		}
	}
